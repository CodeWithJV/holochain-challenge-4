    ...
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::StoreEntry(store_entry) => match store_entry {
            OpEntry::CreateEntry { app_entry, action } => match app_entry {
                EntryTypes::Room(room) => {
                    validate_create_room(EntryCreationAction::Create(action), room)
                }
            ...
            OpEntry::UpdateEntry {
                ...
                EntryTypes::Room(room) => {
                    validate_create_room(EntryCreationAction::Update(action), room)
                }
                EntryTypes::Message(message) => {
            ...
            OpUpdate::Entry { app_entry, action } => {
                ...
                match app_entry {
                    ...
                    EntryTypes::Room(room) => {
                        let original_app_entry =
                            must_get_valid_record(action.clone().original_action_address)?;
                        let original_room = match Room::try_from(original_app_entry) {
                            Ok(entry) => entry,
                            Err(e) => {
                                return Ok(ValidateCallbackResult::Invalid(format!(
                                    "Expected to get Room from Record: {e:?}"
                                )));
                            }
                        };
                        validate_update_room(action, room, original_create_action, original_room)
                    }
                }
            }
            _ => Ok(ValidateCallbackResult::Valid),
        ...
        FlatOp::RegisterDelete(delete_entry) => {
            ...
            match original_app_entry {
                ...
                EntryTypes::Room(original_room) => validate_delete_room(
                    delete_entry.clone().action,
                    original_action,
                    original_room,
                ),
        ...
        FlatOp::RegisterCreateLink {
            ...
            LinkTypes::CreatorToRooms => {
                validate_create_link_creator_to_rooms(action, base_address, target_address, tag)
            }
            LinkTypes::AllRooms => {
                validate_create_link_all_rooms(action, base_address, target_address, tag)
            }
            LinkTypes::CreatorToMessages => {
                validate_create_link_creator_to_messages(action, base_address, target_address, tag)
            }
            LinkTypes::RoomToMessages => {
                validate_create_link_room_to_messages(action, base_address, target_address, tag)
            }
            LinkTypes::MemberToRooms => {
                validate_create_link_member_to_rooms(action, base_address, target_address, tag)
            }
            LinkTypes::RoomToMembers => {
                validate_create_link_room_to_members(action, base_address, target_address, tag)
            }
        },
        FlatOp::RegisterDeleteLink {
            ...
            LinkTypes::CreatorToRooms => validate_delete_link_creator_to_rooms(
                action,
                original_action,
                base_address,
                target_address,
            ...
            LinkTypes::AllRooms => validate_delete_link_all_rooms(
                action,
                original_action,
                base_address,
                target_address,
            ...
            LinkTypes::CreatorToMessages => validate_delete_link_creator_to_messages(
                action,
                original_action,
                base_address,
                target_address,
            ...
            LinkTypes::RoomToMessages => validate_delete_link_room_to_messages(
                action,
                original_action,
                base_address,
                target_address,
            ...
            LinkTypes::MemberToRooms => validate_delete_link_member_to_rooms(
                action,
                original_action,
                base_address,
                target_address,
            ...
            LinkTypes::RoomToMembers => validate_delete_link_room_to_members(
                action,
                original_action,
                base_address,
                target_address,
        ...
        FlatOp::StoreRecord(store_record) => {
            match store_record {
                ...
                OpRecord::UpdateEntry {
                    ...
                    match app_entry {
                        EntryTypes::Room(room) => {
                            let result = validate_create_room(
                                EntryCreationAction::Update(action.clone()),
                                room.clone(),
                            )?;
                            if let ValidateCallbackResult::Valid = result {
                                let original_room: Option<Room> = original_record
                                    .entry()
                                    .to_app_option()
                                    .map_err(|e| wasm_error!(e))?;
                                let original_room = match original_room {
                                    Some(room) => room,
                                    None => {
                                        return Ok(
                                ...
                                validate_update_room(action, room, original_action, original_room)
                            } else {
                                Ok(result)
                            }
                        }
                ...
                OpRecord::DeleteEntry {
                    ...
                    match original_app_entry {
                        EntryTypes::Room(original_room) => {
                            validate_delete_room(action, original_action, original_room)
                        }
                ...
                OpRecord::CreateLink {
                    ...
                    LinkTypes::CreatorToRooms => validate_create_link_creator_to_rooms(
                        action,
                        base_address,
                        target_address,
                        tag,
                    ...
                    LinkTypes::CreatorToMessages => validate_create_link_creator_to_messages(
                        action,
                        base_address,
                        target_address,
                        tag,
                    ),
                    LinkTypes::RoomToMessages => validate_create_link_room_to_messages(
                        action,
                        base_address,
                        target_address,
                        tag,
                    ),
                    LinkTypes::MemberToRooms => validate_create_link_member_to_rooms(
                        action,
                        base_address,
                        target_address,
                        tag,
                    ),
                    LinkTypes::RoomToMembers => validate_create_link_room_to_members(
                        action,
                        base_address,
                        target_address,
                        tag,
